...
function LoginContent() {
    ...
    const [otpSent, setOtpSent] = useState(false); // Using this for "Link Sent" state
    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        try {
            if (!otpSent) {
                // Phase 1: Send Magic Link via Google Script Proxy Cluster
                ...
                // Phase 2: Login after verification
                ...
                toast.success(mode === "login" ? "Welcome back! 🚀" : "Verification complete! 🌟");
                router.push(isOfficial ? "/dashboard" : "/");
            }
        } catch (error: any) {
        ...
        >
            ...
            <div className="text-center mb-8">
                ...
                <h2 className="text-2xl font-bold text-white">
                    {otpSent ? "Check Your Email" : (mode === "login" ? "Welcome Back" : "Join Connect")}
                </h2>
                <p className="text-zinc-500 text-sm mt-1">
                    {otpSent ? `We sent a verification link to ${email}` : (mode === "login" ? "Login to your account" : "Create a new account for free")}
                </p>
                {success && <p className="text-green-400 text-xs mt-2 font-medium">✅ {success}</p>}
            ...
            <form onSubmit={handleAuth} className="space-y-4">
                    ...
                    {!otpSent ? (
                        ...
                        >
                            <p className="text-zinc-400 text-sm">
                                Bhai, mail pe link bhej di hai. <br /> Use click karkey yahan aao aur <br /> <b>"Login"</b> dabao!
                            </p>
                            <button
                                type="button"
                                onClick={() => setOtpSent(false)}
                                className="text-zinc-500 hover:text-white text-xs transition-colors text-center w-full mt-4 underline underline-offset-4"
                            >
                                Change email?
                            </button>
                        </motion.div>
                ...
                >
                    {loading ? (
                        ...
                        <>
                            {!otpSent ? "Send Magic Link" : (mode === "login" ? "Login Now" : "Complete Signup")}
